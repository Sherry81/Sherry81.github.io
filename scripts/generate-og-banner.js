...
const W = 1200;
const H = 630;
const AV = 320; // avatar diameter
const AV_X = 760; // avatar left
const AV_Y = (H - AV) / 2;
...
<svg xmlns="http://www.w3.org/2000/svg" width="${W}" height="${H}" viewBox="0 0 ${W} ${H}">
  ...
  <!-- avatar ring -->
  <circle cx="${AV_X + AV / 2}" cy="${AV_Y + AV / 2}" r="${AV / 2 + 10}" fill="none" stroke="url(#ring)" stroke-width="8"/>
</svg>
`);

const circleMask = Buffer.from(
  `<svg xmlns="http://www.w3.org/2000/svg" width="${AV}" height="${AV}"><circle cx="${AV /
    2}" cy="${AV / 2}" r="${AV / 2}" fill="#fff"/></svg>`
);
